...
process.env.PORT = process.env.PORT ? process.env.PORT : '9270';
...
gulp.task('build', gulp.series(
	clean,
	gulp.parallel(scss, scssComponents, tsSrc),
	assets,
	index
	//typedoc
));
...
function scss() {
	return gulp.src(['src/**/*.{scss,sass}', '!src/**/component-style.{scss,sass}'], { base: 'src/assets/scss' })
...
function scssComponents() {
	return gulp.src('src/**/component-style.{scss,sass}', { base: 'src' })
		.pipe(plugins.sassLint({ config: '.sass-lint.yml' }))
		.pipe(plugins.sassLint.format())
		.pipe(plugins.sassLint.failOnError())
		// .pipe(plugins.rename({ dirname: '' }))
...
function assets() {
	var images = gulp.src('src/assets/images/**/*.{png,jpg,gif}')
		.pipe(plugins.size({ title: 'images' }))
		.pipe(gulp.dest('build/assets/images'));

	var fonts = gulp.src('src/assets/fonts/**/*.{eot,ttf,otf,woff}')
...
function watch() {
	// gulp.watch('src/**/*.{ts,css,html}', gulp.series(tsSrc, 'unit'));
	gulp.watch('src/**/*.{ts,css,html}', tsSrc);
	gulp.watch(['src/**/*.{scss,sass}', '!src/**/component-style.{scss,sass}'], scss);
	gulp.watch('src/**/component-style.{scss,sass}', scssComponents);
